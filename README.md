...
```
Frames render to `./renders/panel_XX.png` alongside your current working directory. No existing `.blend` file is required because the script builds the scene from scratch (clears objects/worlds, then creates a fresh camera, paired key/fill lights, and an emissive text object programmatically so panels stay bright even with a transparent film).
